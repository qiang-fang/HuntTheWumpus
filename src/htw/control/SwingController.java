...
import htw.model.TypeOfDifficulty;
import htw.model.TypeOfDirection;
import htw.model.TypeOfLocation;
...
public class SwingController implements Controller {
  ...
  private int percentageOfPits;
  private int percentageOfBats;
  ...
  private void processMazeArguments(String numberOfPlayers, String wrappingType,
                                    ...
                                    String startingPoint, String percentageOfPits,
                                    String percentageOfBats, String numberOfArrow,
    ...
    if (percentageOfPits.isEmpty()) {
      percentageOfPits = "10";
    }
    if (percentageOfBats.isEmpty()) {
      percentageOfBats = "15";
    }
    ...
    this.percentageOfPits = Integer.parseInt(percentageOfPits);
    this.percentageOfBats = Integer.parseInt(percentageOfBats);
  ...
  private void constructMaze() {
    if (wrappingType.equals("non-wrapping")) {
      ...
      this.model = new NonPerfectMaze(numberOfRows, numberOfColumns, startingPoint,
              percentageOfPits, percentageOfBats, numberOfArrow, seed, numberOfPlayers,
  ...
  public void createMaze(String numberOfPlayers, String wrappingType,
    ...
    try {
      processMazeArguments(numberOfPlayers, wrappingType, numberOfRows, numberOfColumns,
              startingPoint, percentageOfPits, percentageOfBats, numberOfArrow,
  ...
  private void afterMove() {
    view.refresh();
    view.resetFocus();
    if (model.getCurrentDifficulty().get(TypeOfDifficulty.BAT)) {
    ...
    if (model.getCurrentDifficulty().get(TypeOfDifficulty.WUMPUS)) {
      model.setDead();
      view.killedByWumpus();
      view.refreshAfterDie();
      return;
    }
    if (model.getCurrentDifficulty().get(TypeOfDifficulty.PIT)) {
      model.setDead();
      view.fallInPit();
      view.refreshAfterDie();
    }
  ...
  public void processPopup(Component component, int id) {
    if (id == model.getPlayerCurrentLocationId()
            && model.getCurrentLocationType() == TypeOfLocation.CAVE) {
  ...
  @Override
  public void shoot() {
    TypeOfDirection[] directionOptions = model.getPossibleMove(model.getPlayerCurrentLocationId())
            .toArray(new TypeOfDirection[0]);
    TypeOfDirection selectedDirection = view.showShootDirectionDialog(directionOptions);
