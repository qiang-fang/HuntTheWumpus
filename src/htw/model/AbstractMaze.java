...
/**
 * This abstract class contains all the fields and methods that all concrete maze classes are
 * sharing.
 */
public abstract class AbstractMaze implements Maze {
   ...
   * @param seed             same seed will guarantee getting same sequence of random number in
   *                         different instances of Random
   * @throws IllegalArgumentException if numberOfRows or numberOfColumns is less than or equal to 0,
   *                                  if goal or startingPoint is out of range
   */
  protected AbstractMaze(int numberOfRows, int numberOfColumns, int startingPoint,
                         int percentageOfPits, int percentageOfBats, int numberOfArrow,
                         long seed, int numberOfPlayers)
          throws IllegalArgumentException {
    if (numberOfRows <= 0 || numberOfColumns <= 0) {
      throw new IllegalArgumentException(
              "Number of rows and number of columns should be greater than 0");
    }
    ...
    if (percentageOfPits < 0 || percentageOfPits >= 100) {
      throw new IllegalArgumentException(
              "Percentage of pits is out of range");
    }
    if (percentageOfBats < 0 || percentageOfBats >= 100) {
      throw new IllegalArgumentException(
              "Percentage of bats is out of range");
    }
    if (numberOfArrow <= 0) {
      throw new IllegalArgumentException(
              "Number of arrows should be greater than 0");
    }
    ...
    this.turn = 0;
    this.numberOfPits = (int) round(n * percentageOfPits / 100.0);
    this.numberOfBats = (int) round(n * percentageOfBats / 100.0);
  ...
  protected void createWalls(int numberOfRows, int numberOfColumns) {
    ...
    for (int i = 0; i < numberOfRows; i++) {
      for (int j = 0; j < numberOfColumns; j++) {
        ...
        if (i != numberOfRows - 1) {
          walls.add(new Edge(coordinateToId(i, j),
                  coordinateToId(i, j) + numberOfColumns));
        }
      }
    }
  }
  ...
  @Override
  public List<TypeOfDirection> getPossibleMove(int currentId) {
    List<TypeOfDirection> result = new ArrayList<>();
  ...
  @Override
  public TypeOfLocation getCurrentLocationType() {
    return players[turn].currentLocation.type;
  }

  @Override
  public void move(TypeOfDirection direction) throws IllegalArgumentException {
  ...
  @Override
  public String movePlayerTo(TypeOfDirection direction) throws IllegalArgumentException {
    if (!getPossibleMove(players[turn].currentLocation.id).contains(direction)) {
      throw new IllegalArgumentException("Player can't go to that direction");
    }
    ...
    while (true) {
      ...
      if (tmpLocation.difficulty.get(TypeOfDifficulty.WUMPUS)) {
        output.append("Chomp, chomp, chomp, thanks for feeding the Wumpus!\n");
        output.append("Better luck next time!\n");
        ifGameEnd = true;
        break;
      } else if (tmpLocation.difficulty.get(TypeOfDifficulty.PIT)) {
        if (tmpLocation.difficulty.get(TypeOfDifficulty.BAT)) {
        ...
        ifGameEnd = true;
        break;
      } else if (tmpLocation.difficulty.get(TypeOfDifficulty.BAT)) {
        int chance = random.nextInt(2);
        if (chance == 0) {
          tmpId = pickByBats(tmpId);
          output.append("Snatch -- you are grabbed by superbats and ...\n");
    ...
    players[tmpId].currentLocation = locations[tmpId];
  ...
  public int encounterBats() {
    ...
    if (chance == 0) {
      ...
      while (newId == currentId || locations[newId].type != TypeOfLocation.CAVE) {
  ...
  /**
   * Return the caves that can be arrived with one move from the location of the id.
   *
   * @param id        current location id
   * @param direction arrive at a cave with one move from this direction
   * @return an array, first element is previous location id second element is the current cave id
   */
  protected int[] nextCave(int id, TypeOfDirection direction) {
    int[] result = new int[2];
    int tmpId = direction2Id(locations[id], direction);
    int previousId = id;
    Location tmpLocation = locations[tmpId];
    while (tmpLocation.type == TypeOfLocation.TUNNEL) {
  ...
  public boolean[] smellAndFeel(int id) {
    if (locations[id].type != TypeOfLocation.CAVE) {
      return null;
    }
    boolean[] result = new boolean[2];
    result[0] = false;  // smell wumpus
    result[1] = false;  // feel draft
    for (TypeOfDirection direction : getPossibleMove(id)) {
      int smellCave = nextCave(id, direction)[1];
      if (locations[smellCave].difficulty.get(TypeOfDifficulty.WUMPUS)) {
        result[0] = true;
      }
      if (locations[smellCave].difficulty.get(TypeOfDifficulty.PIT)) {
        if (!result[1]) {
          result[1] = true;
        }
      }
  ...
  @Override
  public String smell(List<TypeOfDirection> listOfDirection) {
    int id = players[turn].currentLocation.id;
    boolean smellWumpus = false;
    boolean feelDraft = false;
    StringBuilder result = new StringBuilder();
    for (TypeOfDirection direction : listOfDirection) {
      int smellCave = nextCave(id, direction)[1];
      if (locations[smellCave].difficulty.get(TypeOfDifficulty.WUMPUS)) {
        smellWumpus = true;
  ...
  @Override
  public int[] shoot(TypeOfDirection direction, int distance) throws IllegalArgumentException {
    // result[0]:
    // 0: run out of arrows
    // 1: hit the wall
    // 2: kill the wumpus
    ...
    if (distance == 0) {
      if (locations[currentId].difficulty.get(TypeOfDifficulty.WUMPUS)) {
  ...
  public String updateAndReturnGameResult() {
    ...
    for (int i = 0; i < numberOfPlayers; i++) {
      if (players[i].isWin) {
        ifGameEnd = true;
        return "Player " + (i + 1) + " win the game!!";
      }
      ifAllDead = ifAllDead && (!players[i].isAlive);
    }
    if (ifAllDead) {
      ifGameEnd = true;
      return "All players are dead, Wumpus rules the world, game over!!";
    }
    return "";
  }
  ...
  protected TypeOfDirection id2Direction(int from, int to) {
  ...
  protected void addSideWalls() {
    if (numberOfColumns > 2) {
      for (int i = 0; i < numberOfRows; i++) {
        this.walls.add(new Edge(i * numberOfColumns + numberOfColumns - 1,
                i * numberOfColumns));
      }
    }
    if (numberOfRows > 2) {
  ...
  protected void buildNonPerfectMaze() {
    ...
    setLocationType();
    setDifficulty(NUMBER_OF_WUMPUS, TypeOfDifficulty.WUMPUS);
    setDifficulty(numberOfPits, TypeOfDifficulty.PIT);
    setDifficulty(numberOfBats, TypeOfDifficulty.BAT);
  }

  protected void setLocationType() {
    for (Location l : locations) {
      if (l.adjacencyList.size() == 2) {
        l.type = TypeOfLocation.TUNNEL;
      } else {
        l.type = TypeOfLocation.CAVE;
      }
    }
  }

  protected void setDifficulty(int number, TypeOfDifficulty difficultyName) {
    int count = 0;
    while (count < number) {
      int id = random.nextInt(n);
      if (id != startingPoint && locations[id].type == TypeOfLocation.CAVE
              && !locations[id].difficulty.get(difficultyName)) {
        locations[id].difficulty.put(difficultyName, true);
        count++;
      }
    }
  }
  ...
  @Override
  public Map<TypeOfDifficulty, Boolean> getCurrentDifficulty() {
    return players[turn].currentLocation.difficulty;
  }

  @Override
  public Map<TypeOfDifficulty, Boolean> getCurrentDifficulty(int id) {
    return locations[id].difficulty;
  }
  ...
  public String toString() {
    ...
    for (Location l : locations) {
      result.append("difficulty of cell:").append(l.id).append(": ").append(l.difficulty)
              .append("\n");
      result.append("adjacency list of cell:").append(l.id).append(": ")
              .append(l.adjacencyList).append("\n");
    }
    return result.toString();
  }
}
