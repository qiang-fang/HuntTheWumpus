...
/**
 * This interface contains all operations that all types of mazes should support.
 */
public interface Maze extends ReadonlyModel {
   ...
   */
  String movePlayerTo(TypeOfDirection direction);
   ...
   * @param direction the direction the player will move to
   */
  void move(TypeOfDirection direction);
  ...
  /**
   * A player can attempt to slay the Wumpus by specifying a direction and distance in which to
   * shoot their crooked arrow. Distance is defined as the number of caves (but not tunnels) that an
   * arrow travels. Arrows travel freely down tunnels (even crooked ones) but only travel a straight
   * line through a cave. Return if it can kill the Wumpus.
   *
   ...
   */
  int[] shoot(TypeOfDirection direction, int distance);

  /**
   * Return the id of location to which bats take the player.
   *
   * @return the id of location to which bats take the player, -1 means player ducked bats
   */
  int encounterBats();
